...
void mission(Student s, string phone, string id, string filename) {
    ofstream file(filename, std::ios_base::app);
    if (s.student_number == id) {
        file << s.student_number << ";" << s.user_id << ";" << s.name
             << ";" << phone << ";" << s.email << ";" << s.skype << endl;
    } else {
        file << s.student_number << ";" << s.user_id << ";" << s.name
             << ";" << s.phone_number << ";" << s.email << ";" << s.skype << endl;
    }
    file.close();
}
...
int main() {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            string new_number;
            string number = parts.at(1);
            cout << "Enter a new phone number: ";
            getline(cin,new_number);
            if (is_valid_phone_number(new_number)) {
                std::ofstream asd(file_name, std::ofstream::trunc);
                for(auto pair: user_ids) {
                    mission(*(pair.second), new_number, number, file_name);
                }
                for(auto pair: student_numbers) {
                    pair.second = nullptr;
                }
                ...
                read_data(file_name, user_ids, student_numbers);
